...
const ApplicationDetails = () => {
        ...
        if (stepIdToComplete === 'CREDIT_CHECK') {
            if (!cibilScore.trim() || isNaN(Number(cibilScore))) {
                toast({
                    variant: "destructive",
                    title: "Invalid Input",
                    description: "Please enter a valid CIBIL score.",
                });
                return;
            }
            try {
                ...
                toast({
                    variant: "destructive",
                    title: "API Error",
                    description: err.response?.data?.message || "Failed to run credit check.",
                    duration: 3000,
                });
            }
            return;
            ...
            if (success) {
                ...
                if (stepToCompleteIndex < workflowSteps.length - 1) {
                    ...
                    toast({
                        ...
                        description: `${stepDetails.label} completed successfully.`
                    });
                } else {
                    toast({
                        variant: "success",
                        title: "Workflow Complete",
                        description: "The application process has been fully completed."
                    });
                }
            } else {
                toast({
                    variant: "destructive",
                    title: "Action Failed",
                    description: `Could not complete the action for ${stepDetails.label}. Please try again.`
                });
            }
        } catch (error) {
            toast({
                variant: "destructive",
                title: "API Error",
                description: `An error occurred while processing the action for ${stepDetails.label}.`
            });
        } finally {
    ...
    const handleDigoDocsView = (label, doc) => {
            ...
            .catch(
                (err) => {
                    toast({
                        ...
                        description: err.response.data.message || `Failed to ${status.toLowerCase()} ${verificationDocInfo.type.replace(/_/g, ' ')}. Please try again.`
                    });
                }
            )
    ...
    return (
        <div className={`${styles.mainContainer} scrollContainer`}>
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <Button
                    className={`${getApplicationStatusClasses(applicationData?.applicationStatus)} cursor-auto hover:bg-color-none rounded-lg`}>
                            ...
                            <div className="space-y-2">
                                <div>
                                    ...
                                    <div className="flex items-center space-x-2">
                                        {(() => {
                                            const cibil = applicationData?.cibil;
                                            if (!cibil) {
                                                return (
                                                    <>
                                                        <div className="flex-1 bg-gray-200 rounded-full h-2"/>
                                                        <span
                                                            className="text-sm font-medium text-gray-500">Not Checked</span>
                                                    </>
                                                );
                                            }
                                    ...
                                    <Badge className={`${getStatusColor(applicationData?.borrower?.risk)}`}>
                                        {
                                            applicationData?.borrower?.risk === 'NOT_AVAILABLE'
                                                ? 'N/A'
                                                : applicationData?.borrower?.risk.charAt(0) + applicationData?.borrower?.risk.slice(1).toLowerCase() + ' Risk'
                                        }
                                    </Badge>
                                </div>
                ...
                <div>
                    ...
                    <Card>
                            ...
                            <div className="space-y-4">
                                ...
                                {activeWorkflowTab === 'KYC' && (
                                    <div className='border p-4 rounded-lg'>
                                        ...
                                        <p className="text-[13px] text-gray-500 mb-3">Verify the customer's identity
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="flex items-center justify-between p-3 border rounded-lg">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <Badge
                                                        className={`${detailsVerified('PAN') === 'APPROVED' ? 'bg-green-100 text-green-800'
                                                    ...
                                                    </Badge>
                                                    {
                                                        detailsVerified('PAN') === 'REJECTED' || detailsVerified('PAN') === 'APPROVED' ?
                                                            ...
                                                            <Button variant="outline" size="sm"
                                                                    ...
                                                                    onClick={() => handleDocumentVerification('PAN')}>
                                                                Verify
                                                            </Button>
                                                    }
                                                    {
                                                        applicationData?.loanDocuments.find(
                                                            ...
                                                            <Button variant="ghost" size="sm"
                                                                    onClick={() => handleOpenDocumentPreview('PAN')}>
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    </Badge>
                                                    {
                                                        detailsVerified('AADHAAR') === 'REJECTED' || detailsVerified('AADHAAR') === 'APPROVED' ?
                                                            ...
                                                            <Button variant="outline" size="sm"
                                                                    className='text-xs bg-primary hover:bg-color-none hover:text-white-100'
                                                                    onClick={() => handleDocumentVerification('AADHAAR')}>
                                                                Verify
                                                            </Button>
                                                    }
                                                    {
                                                        applicationData?.loanDocuments.find(
                                                            ...
                                                            <Button variant="ghost" size="sm"
                                                                    onClick={() => handleOpenDocumentPreview('AADHAAR')}>
                                                                <Eye className="w-4 h-4"/>
                                                            </Button>
                                                            : ''
                                                    }
                                            ...
                                            <div className="flex items-center justify-between p-3 border rounded-lg">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <Badge
                                                        className={`${detailsVerified('SALARY_SLIP') === 'APPROVED' ? 'bg-green-100 text-green-800'
                                                            :
                                                    ...
                                                    <Button variant="ghost" size="sm"
                                                            onClick={() => handleOpenDocumentPreview('SALARY_SLIP')}>
                                                        <Eye className="w-4 h-4"/>
                                                    </Button>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between p-3 border rounded-lg">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <Badge
                                                        className={`${detailsVerified('BANK_STATEMENT') == 'APPROVED' ? 'bg-green-100 text-green-800'
                                                            :
                                ...
                                {activeWorkflowTab === 'CREDIT_CHECK' && (
                                    <div className='border p-4 rounded-lg'>
                                        <h4 className="text-sm font-medium text-black-600 mb-2">Credit Check & Risk
                                            Assessment</h4>
                                        <p className="text-[13px] text-gray-500 mb-3">Enter the CIBIL score and perform
                                            a credit check to assess the borrower's creditworthiness.</p>

                                        <div className="mt-4 max-w-sm">
                                            <label htmlFor="cibilScore"
                                                   className="block text-sm font-medium text-gray-700 mb-1">
                                                CIBIL Score <sup className="text-red-500">*</sup>
                                            </label>
                                            <input
                                                id="cibilScore"
                                                type="number"
...

                                        {
                                            !isProceeding && activeWorkflowTab === 'CREDIT_CHECK' ?
                                                <Button
                                                    ...
                                                    disabled={
                                                        workflowSteps.findIndex(s => s.id === 'CREDIT_CHECK') !== highestCompletedStepIndex + 1 ||
                                                        !cibilScore.trim()
                                                    }
                                                >
                                                ...
                                                <Button variant='outline'
                                                        className='mt-4 bg-purple-100 hover:bg-color-none text-purple-600 hover:text-purple-600 flex items-center space-x-2 cursor-not-allowed'
                                                        disabled>
                                                    Processing...
                                                </Button>
                                        }
                                ...
                                {activeWorkflowTab === 'UNDERWRITING' && (
                                    <div className='border p-4 rounded-lg'>
                                        <h4 className="text-sm font-medium text-black-600 mb-2">Digital
                                            Underwriting</h4>
                                        ...
                                        {applicationData?.underwriting === 'PENDING' && (
                                            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                                                <div
                                                    className="flex items-center gap-2 text-yellow-700 text-sm font-medium">
                                        ...
                                        {
                                            (applicationData?.loanWorkflow.UNDERWRITING || applicationData?.underwriting === 'COMPLETED') ?
                                                ""
                                                :
                                                <div className="mt-4 flex flex-col space-y-2">
                                                    <label className='text-sm font-medium text-black-600'>Borrower's
                                                        Email Address</label>
                                                    <input
                                                        id="username"
                                                        type="text"
                                                        ...
                                                        disabled={applicationData?.underwriting === 'PENDING' || applicationData?.underwriting === 'COMPLETED'}
                                                    />
                                                </div>
                                        }
                                        {
                                            (applicationData?.loanWorkflow.UNDERWRITING || applicationData?.underwriting === 'COMPLETED') && activeWorkflowTab === 'UNDERWRITING' ?
                                                <div
                                                    className="flex items-center gap-2 text-green-600 text-[14px] mt-4 font-medium">
                                                        ...
                                                        <div className="text-amber-800 font-sm mb-2">
                                                            {
                                                                applicationData?.approvalConditions === null ?
                                                                    <span
                                                                        className='flex items-center gap-2 text-[14px] font-medium'> <CheckCircle
                                                                        className="w-4 h-4"/> Approved</span>
                                                                    :
                                                                    <span
                                                                        className='flex items-center gap-2 text-[14px] font-medium'> <ShieldCheck
                                                                        className="w-4 h-4"/> Approved with Conditions</span>
                                                            }
                                                        </div>
                                                        {
                                                            applicationData?.approvalConditions != null ?
                                                                <div className='mt-2'>
                                                                    ...
                                                                    <div className="text-sm text-amber-800"
                                                                         dangerouslySetInnerHTML={{__html: applicationData?.approvalConditions}}></div>
                                                                </div>
                                                                :
                                                                ""
                                                        }
                      ...
                      border rounded-md p-2 text-[13px]' placeholder='Enter your remark about this application'
                                                                  value={decessionRemark}
                                                                  onChange={(e) => setDecessionRemark(e.target.value)}
                                                                  rows={5}></textarea>
                                                    </div>
